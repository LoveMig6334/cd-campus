-- ----------------------------------------------------------------------------
-- Clock stop (timed sports): freeze the game clock on a dead ball without
-- leaving 'live' — no status change, so the hall board shows no pause splash
-- and scoring / fouls stay open (free throws). Two new events toggle a flag;
-- start / resume / end_set clear it so a new period always begins running.
-- ----------------------------------------------------------------------------

alter type match_event_type add value if not exists 'clock_stop';
alter type match_event_type add value if not exists 'clock_start';

alter table matches
  add column clock_stopped boolean not null default false;

-- Same body as 0016; the clock now runs only while live AND not stopped.
...
begin
  ...
  -- Dead-ball clock stop flag.
  v_stopped := case
    when p_type::text = 'clock_stop' then true
    when p_type::text in ('clock_start','start','resume','end_set') then false
    else v_match.clock_stopped end;

  -- Game clock: runs while the match is live and not stopped, accumulates
  -- otherwise. The shot clock follows it: unfreeze when the clock starts,
  ...
  if p_status = 'live' and not v_stopped then
