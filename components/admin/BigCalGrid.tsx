...
export function BigCalGrid({ days }: { days: BigCalDay[] }) {
  return (
    <div className="border-ink bg-paper grid grid-cols-7 border-[1.5px]">
      <div className="bg-ink text-yellow col-span-7 grid grid-cols-7 font-mono text-[10px] tracking-[0.18em]">
        {WEEKDAYS.map((d) => (
      ...
      {days.map((day, i) => (
        <div
          key={i}
          className={cn(
            "border-mute-200 relative aspect-[1/0.85] overflow-hidden border-r border-b border-dashed px-2 py-1.5",
            (i + 1) % 7 === 0 && "border-r-0",
        ...
        >
          ...
          {day.events?.map((event, j) => (
            <div
              key={j}
              className="mt-1 overflow-hidden px-1 py-0.5 font-mono text-[9px] text-ellipsis whitespace-nowrap"
              style={{
